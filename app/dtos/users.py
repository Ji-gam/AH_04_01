...
class UserUpdateRequest(BaseModel):
    ...
    name: Annotated[str | None, Field(None, description="Profile의 이름", min_length=2, max_length=20, examples=["홍길동"])]
    email: Annotated[
        EmailStr | None,
        Field(None, description="변경할 이메일. 다른 계정이 이미 쓰고 있으면 409를 반환한다.", max_length=40, examples=["new@example.com"]),
    ]
...
class UserInfoResponse(BaseSerializerModel):
    ...
    profile_id: Annotated[int, Field(description="Profile(개인정보)의 PK. 앞으로 추가되는 도메인 API는 이 값을 기준으로 조회/저장한다.")]
