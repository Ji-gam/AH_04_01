...
async def test_parenthesized_ingredient_line_is_not_treated_as_a_drug_name():
    ...
    안 된다."""
    dur_repo = _FakeDurDrugRepository([("KD_PLAV001", "플라빅스정75mg"), ("KD_CLOP001", "클로피도그렐 75mg")])
    ...
    assert "KD_PLAV001" in matched_item_seqs
    assert "KD_CLOP001" not in matched_item_seqs, "괄호 성분표기 줄이 별도 약품으로 잘못 매칭됨"
...
async def test_drug_name_without_dosage_unit_still_matches_by_form_suffix():
    ...
    dur_repo = _FakeDurDrugRepository([("KD_SERE001", "세레타이드500디스커스")])
    ...
    matched_item_seqs = {d.item_seq for d in matched}
    assert "KD_SERE001" in matched_item_seqs
...
async def test_all_six_prescription_items_are_recognized():
    ...
    dur_repo = _FakeDurDrugRepository([(f"KD_RX{i:03d}", name) for i, name in enumerate(names)])
    ...
    matched_item_seqs = {d.item_seq for d in matched}
    assert {f"KD_RX{i:03d}" for i in range(len(names))} <= matched_item_seqs
...
async def test_drug_with_manufacturer_bracket_and_non_mg_dosage_is_matched():
    """제형 접미사만으로 후보 인정된 약도, 실제 매칭/자동 생성 흐름까지 이어지는지 확인한다."""
...
async def test_fuzzy_match_rescues_clova_character_misread():
    ...
    들어온다 — 서버 로그로 확인). 이런 한 글자 오인식은 마스터 데이터 약품명과의 유사도 비교로
    구제돼야 한다."""
    dur_repo = _FakeDurDrugRepository([("KD_NOSP002", "노스판패취10㎍/h")])
...
async def test_fuzzy_match_does_not_trigger_on_unrelated_prescription_text():
    ...
    fallback으로는 나올 수 있지만, 그 경우 confidence 맵에는 없어야 한다 — 실제 유사도 매칭으로
    인정된 게 아니라는 뜻)"""
    dur_repo = _FakeDurDrugRepository([("KD_NOSP003", "노스판패취10㎍/h")])

    async with TestSessionLocal() as session:
        _matched, _auto_created_ids, match_confidence = await medication_service._match_or_create_medications(
            session,
            dur_repo,
