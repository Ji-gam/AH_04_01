...
def test_compute_match_rate_uses_confidence_directly_for_known_drug():
    """(T-MED-6) 마스터 DB에 이미 있는 약은 하드코딩된 값(1.0/0.85) 대신 실제 OCR confidence를
...
async def test_match_or_create_medications_returns_ocr_confidence_for_existing_match():
    ...
    async with TestSessionLocal() as session:
        med = await repo.create_medication(
            session, Medication(standard_code="KD_CONF001", medication_name="컨피던스정100mg", form_type="TABLET")
        )

        matched, auto_created_ids, match_confidence = await medication_service._match_or_create_medications(
            session, repo, [OcrField(text="*컨피던스정100mg", confidence=0.63)]
        )

    assert any(m.id == med.id for m in matched)
    assert med.id not in auto_created_ids
    assert match_confidence[med.id] == 0.63


async def test_match_or_create_medications_reports_confidence_for_auto_created_drug(monkeypatch):
    """(T-MED-6) 마스터 DB/공공 API 어디에도 없어 즉석 생성된 약품도 confidence 맵에 포함되어야
    ...
    async with TestSessionLocal() as session:
        matched, auto_created_ids, match_confidence = await medication_service._match_or_create_medications(
            session, repo, [OcrField(text="*완전히새로운약999mg", confidence=0.99)]
        )

    assert len(matched) == 1
    med = matched[0]
    assert med.id in auto_created_ids
    assert match_confidence[med.id] == 0.99
    # 실제 API가 담당하는 상한 적용까지 합쳐서 검증
    assert medication_service._compute_match_rate(match_confidence[med.id], is_auto_created=True) == 0.5


async def test_match_or_create_medications_no_ocr_evidence_fallback_has_no_confidence_entry():
    """(T-MED-6) OCR 텍스트가 약품명처럼 안 보여 마스터 DB 상위 몇 개를 참고용으로만 보여주는
    경우, 그 약들은 confidence 맵에 없어야 한다 — 호출부가 기본값(낮은 match_rate)을 적용할 근거."""
    repo = MedicationRepository()
    async with TestSessionLocal() as session:
        await repo.create_medication(
            session, Medication(standard_code="KD_NOEV001", medication_name="무근거참고약", form_type="TABLET")
        )

        matched, auto_created_ids, match_confidence = await medication_service._match_or_create_medications(
            session, repo, [OcrField(text="환자정보", confidence=0.99)]
        )

    assert len(matched) > 0
    assert all(m.id not in match_confidence for m in matched)
