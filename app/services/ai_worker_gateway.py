...
class AIWorkerGateway:
    ...
    async def upload_csv(self, file_bytes: bytes, filename: str) -> dict:
        """T-ADMIN-1: ai_worker의 `/admin/ingest/csv`로 업로드 파일을 그대로 프록시한다.
        재업로드 자체가 그 파일 소스만 upsert하므로 force 파라미터는 없다(전체 리셋은
        `reset_dur_collection()` 참고)."""
        try:
            async with httpx.AsyncClient(timeout=self._generate_timeout) as client:
                response = await client.post(
                    f"{self._base_url}/admin/ingest/csv",
                    files={"file": (filename, file_bytes, "text/csv")},
                )
        except httpx.HTTPError as e:
            raise AIWorkerUnavailableError(f"ai_worker CSV 인제스트 요청 실패: {e}") from e

        if response.status_code != 200:
            raise AIWorkerUnavailableError(
                f"ai_worker CSV 인제스트 실패(status={response.status_code}): {response.text}"
            )
        return response.json()

    async def reset_dur_collection(self) -> dict:
        try:
            async with httpx.AsyncClient(timeout=self._generate_timeout) as client:
                response = await client.post(f"{self._base_url}/admin/ingest/csv/reset")
        except httpx.HTTPError as e:
            raise AIWorkerUnavailableError(f"ai_worker DUR 컬렉션 리셋 요청 실패: {e}") from e

        if response.status_code != 200:
            raise AIWorkerUnavailableError(
                f"ai_worker DUR 컬렉션 리셋 실패(status={response.status_code}): {response.text}"
            )
        return response.json()

    async def trigger_paper_ingest(self, categories: list[str] | None, retmax_per_category: int | None) -> dict:
        """ai_worker의 `/admin/ingest/papers`(백그라운드 실행, 즉시 반환)로 프록시한다."""
        payload = {"categories": categories, "retmax_per_category": retmax_per_category}
        try:
            async with httpx.AsyncClient(timeout=self._generate_timeout) as client:
                response = await client.post(f"{self._base_url}/admin/ingest/papers", json=payload)
        except httpx.HTTPError as e:
            raise AIWorkerUnavailableError(f"ai_worker 논문 인제스트 요청 실패: {e}") from e

        if response.status_code != 200:
            raise AIWorkerUnavailableError(
                f"ai_worker 논문 인제스트 실패(status={response.status_code}): {response.text}"
            )
        return response.json()

    async def reset_paper_collection(self) -> dict:
        try:
            async with httpx.AsyncClient(timeout=self._generate_timeout) as client:
                response = await client.post(f"{self._base_url}/admin/ingest/papers/reset")
        except httpx.HTTPError as e:
            raise AIWorkerUnavailableError(f"ai_worker 논문 컬렉션 리셋 요청 실패: {e}") from e

        if response.status_code != 200:
            raise AIWorkerUnavailableError(
                f"ai_worker 논문 컬렉션 리셋 실패(status={response.status_code}): {response.text}"
            )
        return response.json()
    ...
    async def ingest_status(self) -> dict:
        try:
            async with httpx.AsyncClient(timeout=self._generate_timeout) as client:
                response = await client.get(f"{self._base_url}/admin/ingest/status")
        except httpx.HTTPError as e:
            raise AIWorkerUnavailableError(f"ai_worker 인제스트 상태 조회 요청 실패: {e}") from e

        if response.status_code != 200:
            raise AIWorkerUnavailableError(
                f"ai_worker 인제스트 상태 조회 실패(status={response.status_code}): {response.text}"
            )
        return response.json()
