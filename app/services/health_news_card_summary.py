...
3. 숫자의 방향(늘었는지 줄었는지)을 기사와 반대로 쓰지 마세요. +/- 부호를 붙일 때는
   기사에서 그 값이 증가한 것인지 감소한 것인지 다시 확인하세요. 방향이 헷갈리면 부호를
   빼고 값만 쓰세요 (예: "63kg").
4. 카드마다 다른 이야기를 담으세요. 같은 내용을 다르게 말한 카드를 두지 마세요.
5. 진단하거나 치료를 권하는 말투를 쓰지 마세요. 기사가 전한 연구 결과를 그대로 옮기세요.
6. icon_key는 반드시 아래 목록의 값 그대로 쓰세요. 목록에 없는 이름은 쓸 수 없습니다.
