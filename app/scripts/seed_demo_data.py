...
class DemoProfileSpec(TypedDict):
    ...
    subtype_habit: SubtypeHabitSpec | None  # 세부진단명이 있을 때만 사용(더미 LLM 습관 캐시)
...
DEMO_PROFILES: list[DemoProfileSpec] = [
    {
        ...
        "subtype_habit": {"label": "식후 30분 걷기", "icon": "🚶", "unit": "분", "target": 30},
    ...
    },
    {
        ...
        "disease_subtype": "협심증",
        "subtype_habit": {"label": "저염식 챙기기", "icon": "🧂", "unit": "회", "target": 1},
...
async def _seed_subtype_habit_cache(session, disease_subtype_id: int, spec: SubtypeHabitSpec) -> None:
    """실제 AIWorkerGateway를 호출하지 않고, 진단명별 습관 캐시(habit_subtype_suggestions)에
    더미 값을 미리 넣어둔다 - 데모 계정으로 추천 목록을 열어봐도 ai_worker 없이 바로 뜬다.
    이미 캐시가 있으면(실제 사용 중 생성됐을 수도 있음) 건드리지 않는다 - unique 제약 위반 방지."""
...
async def seed_demo_data() -> list[str]:
    ...
    async with AsyncSessionLocal() as session:
        for spec in DEMO_PROFILES:
            ...
            if subtype_id is not None and spec["subtype_habit"] is not None:
                await _seed_subtype_habit_cache(session, subtype_id, spec["subtype_habit"])
