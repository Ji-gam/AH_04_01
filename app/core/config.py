...
class Config(BaseSettings):
    ...
    # FCM(Firebase Cloud Messaging) - 앱 패키징 후 iOS/Android 네이티브 푸시용. Firebase
    # 콘솔 > 프로젝트 설정 > 서비스 계정 > "새 비공개 키 생성"으로 받은 json을
    # app/secrets/firebase-service-account.json에 두고 .env에 경로를 넣는다(레포 루트
    # 기준 상대경로 - uv run은 항상 레포 루트에서 실행하므로). 설정 안 하면
    # push_service.py가 iOS/Android 발송만 건너뛰고(웹푸시는 그대로 동작) 조용히 넘어간다.
