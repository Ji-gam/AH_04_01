...
| MySQL 스키마 | `app/models/dur.py`에 20개 테이블 정의(22개 원본 API 테이블 중 두 리포지토리가 실제로 조회하는 것만 — `drug_bundle_info`/`drug_max_dosage`/`drug_prdt_prmsn_list`/`dur_prod_master_list`는 제외), Alembic 리비전 `0024_add_dur_tables.py` | 조인/필터 키(`item_seq`, `ingr_code` 등)만 인덱스가 필요한 `String`, 나머지 원문 텍스트는 SQLite 원본처럼 길이 제한 없는 `Text`. |
