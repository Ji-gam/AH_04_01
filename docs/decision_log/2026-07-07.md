# 2026-07-07 — AH_04_01 이관 및 하네스 문서 정리

> 이 파일은 기존 `docs/decision_log.md`(v1.0~v1.4, 전량 2026-07-07 기록)를
> 날짜별 폴더 구조로 소급 이관한 것이다. 컨벤션은 `docs/decision_log/README.md` 참고.
> (v1.5의 T-MED-3/T-MED-4 관련 내용은 발생일 기준에 맞춰 `docs/decision_log/2026-07-08.md`로 옮겼다.)

## AH_04_01 이관 시 변경된 결정
...
| `docs/` → `docs/dev/`(개발설계 산출물) + `docs/plan/`(기획 문서)로 재구성 | `ERD.dbml`, `api_spec_core_v1_v1.1.yaml`, `sample_code_chat/`, `sample_code_recog/`는 `docs/dev/`로, `PRD_ReMedi_v1.1.md`, `TRD_ReMedi_v1.1.md`, 요구사항 정의서 xlsx는 `docs/plan/`으로 이동 | 문서가 늘어나면서 "이게 지금 지켜야 할 규칙인지, 참고용 산출물인지, 기획 원본인지" 구분이 안 됐다. 폴더로 성격을 나누면 파일명은 그대로 유지(문서버전 규칙 준수)하면서도 한눈에 구분된다 |
...
> 2026-07-08에 추가된 미결사항(T-MED-3, T-MED-4)은 `docs/decision_log/2026-07-08.md` 참고.
