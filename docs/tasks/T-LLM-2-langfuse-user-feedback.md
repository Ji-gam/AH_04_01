...
- [x] ai_worker가 `{"type":"trace","trace_id":...}` 청크를 내보낸다 (Langfuse 미설정 시 생략)
- [x] `chat_messages.trace_id` 저장됨
- [x] `chat_message_feedbacks` 테이블 생성, message_id UNIQUE
- [x] `POST /chat/messages/{id}/feedback` 동작 + 소유권 검증
- [x] `ChatMessageResponse.id` 노출 → 과거 대화에도 피드백 가능
...
### 실제 사례 (2026-08-03, 로컬 실측)

대표 질문 15개(DUR 병용/약효 질문, 5대 질환 관련 질문, 일반 잡담 대조군 1개)를 실제로
던지고 👍/👎를 눌러 데이터를 만들었다. 이 과정에서 검색 자체는 정상(임계값 0.35 안쪽,
관련 문서 확보)인데 **답변이 혼란스러운** 사례를 하나 발견해 고쳤다.

- **질문**: "졸피뎀 장기 복용하면 위험한가요?" (일반 성인 기준 질문, 임신 여부 언급 없음)
- **낮은 점수 원인**: trace [`6f24a2c194fc527786c6201685425d71`](https://jp.cloud.langfuse.com/project/cms2noxdb000aad0hmxki7460/traces/6f24a2c194fc527786c6201685425d71)
  에서 검색 자체는 정확했다(유사도 0.28~0.30으로 임계값 0.35 안쪽, 무관한 문서가 섞인 게
  아님). 문제는 **검색된 3개 문서 중 "임부금기의약품" 문서가 가장 유사도가 높았는데**, 이
  문서 내용이 "임신 후반기 신생아 금단 증상·저체온증·수유 장애"처럼 **임산부/신생아
  한정** 경고였다는 점이다. 시스템 프롬프트가 "질문자 본인이 임신부/노인이 아닌데 질문
  대상이 그런 경우"만 구분하도록 되어 있었지, **참고 문서 자체가 특정 대상 한정 경고일
  때 그걸 명시하라는 지시가 없어서**, LLM이 이 임신부 전용 경고를 일반 성인 질문에
  구분 없이 그대로 섞어 넣어 답변이 문맥상 혼란스러워졌다(👎, 코멘트: "질문은 일반 성인의
  장기복용 위험인데, 답변이 임신부/신생아 관련 내용을 구분 없이 섞어서 혼란스러움").
- **조치**: `ai_worker/tasks/chat_agent.py`의 `_SYSTEM_PROMPT_TEMPLATE`에 문장 추가 —
  "참고 문서 중 임산부·노인·소아 등 특정 대상에게만 해당하는 경고가 있다면, 그 경고가
  어떤 대상에 한정되는지 먼저 밝히고 질문자가 실제로 묻는 대상에 적용되는 내용인지
  구분해서 설명하라"는 지시를 추가했다. **임계값(DUR 0.35 / 논문 0.40)은 전혀 건드리지
  않았다** — 검색 자체는 문제가 없었기 때문에 발표자료의 실측 수치와 어긋나지 않는다.
- **결과**: 같은 질문을 다시 던지자(trace [`f0c9946128f0953fa37cf75cfc0938ea`](https://jp.cloud.langfuse.com/project/cms2noxdb000aad0hmxki7460/traces/f0c9946128f0953fa37cf75cfc0938ea),
  검색된 문서는 이전과 동일) 답변이 "졸피뎀은 최대 28일 복용 권장, 장기 복용 시
  의존성·금단 증상·정신신경계 이상반응 위험 증가"로 바뀌어, 신생아/임신 관련 무관한
  내용 없이 질문자가 실제로 물은 "일반 성인의 장기 복용 위험"만 정확하게 설명했다(👍).
