...
> - v1.3 (2026-07-09): 로컬/실시간 구분은 사용자에게 굳이 필요 없다는 피드백으로 되돌림. `source`
>   필드는 제거하고, 모든 결과에 고정 문구 "출처: 식약처 의약품안전나라(DUR·의약품 개요정보)"만 표시.
...
- [ ] 결과가 있을 때도 "출처: 식약처 의약품안전나라(DUR·의약품 개요정보)" 문구를 같이 밝힌다 —
      "정보 없음"이 아니라 "있는 정보"에도 출처를 밝혀야 신뢰도가 생긴다(로컬 DB인지 실시간 API인지는
      사용자에게 굳이 구분해 보여줄 필요 없다는 피드백에 따라 단일 고정 문구로 통일)
...
- 완료 정의 체크리스트 결과:
  ...
  - [x] 결과가 있을 때 고정 문구 "출처: 식약처 의약품안전나라(DUR·의약품 개요정보)" 노출 (로컬/실시간
    구분 없이 단일 문구 — 처음엔 `source` 필드로 구분해 표시했다가, 사용자 피드백으로 되돌림)
- 가정(Assumptions):
