...
import requests
from config_db import DB_PATH, APISpec
...
class APIPipeline:
    def __init__(self, spec: APISpec, api_key: str, output_dir: str | None = None, multiprocess_mode: bool = False):
        self.spec = spec
    ...
    def _print_progress_bar(
        self, current: int, total: int, start_time: float, prefix_msg: str = "", bar_length: int = 20
    ):
        if total <= 0:
            return
        ...
        if self.multiprocess_mode:
            print(
                f"[{self.spec.name}] {prefix_msg}({percent:.1f}% / {current}건 / 총 {total}건) DB 적재 중... 예상: {eta_str}"
            )
        else:
            msg = f"\r{prefix_msg}[{bar}] ({percent:.1f}% / {current}건 / 총 {total}건) DB 적재 중... 예상: {eta_str}  "
            print(msg, end="", flush=True)

    def _call_api(self, page_no: int, num_of_rows: int, extra_params: dict[str, Any] | None = None) -> str | None:
        params: dict[str, Any] = {"serviceKey": self.api_key, "pageNo": page_no, "numOfRows": num_of_rows}
        ...
        try:
            ...
            if (
                ...
                or "Error forwarding request" in response_text
                or "LIMIT_EXCEEDED" in response_text
                or "TOTAL_TRACK_LIMIT" in response_text
            ):
    ...
    def _get_checkpoint_path(self, extra_label: str = "") -> str:
        return os.path.join(
            self.output_dir,
            f"checkpoint_{self.spec.name}_{extra_label}.json" if extra_label else f"checkpoint_{self.spec.name}.json",
        )
    ...
    def _delete_checkpoint(self, extra_label: str = ""):
        try:
            os.remove(self._get_checkpoint_path(extra_label))
        except Exception:
            pass
    ...
    def _safe_flush(self, conn: sqlite3.Connection, buffer_list: list[dict[str, Any]]) -> None:
        try:
    ...
    def _flush_if_chunk_full(self, conn: sqlite3.Connection, buffer_list: list[dict[str, Any]]) -> None:
        if len(buffer_list) >= self.chunk_size:
            self._safe_flush(conn, buffer_list)

    def _wait_after_error(self, tag: str, message: str):
        print(f"\n⚠️ [{tag}] {message}")
        write_progress_log(tag, "WAITING", message)
        time.sleep(1800)  # 30분 대기

    def _handle_empty_items(
        self, page_no: int, num_of_rows: int, accumulated_rows: int, total_count: int, once: bool, extra_label: str
    ) -> str:
        """items가 비어있을 때의 처리. "break" 또는 "continue" 중 호출부가 취할 동작을 반환한다."""
        ...
        if accumulated_rows < total_count and total_count > 0:
            ...
            mismatch_msg = (
                f"⚠️ 데이터 정합성 불일치 (수집: {accumulated_rows} < 전체: {total_count}) - 30분 대기 후 재시도"
            )
            self._wait_after_error(f"{self.spec.name}{extra_label}[정합성 위배 감지]", mismatch_msg)
            return "continue"

        return "break"

    def _should_stop_paging(self, once: bool, page_no: int, accumulated_rows: int, total_count: int) -> bool:
        # once일 때 최대 10페이지(10회 호출)만 실행하고 중단
        if once and page_no >= self.spec.start_page + 9:
            return True
        return accumulated_rows >= total_count and total_count > 0

    def _maybe_log_heartbeat(
        self,
        last_heartbeat_time: float,
        last_heartbeat_rows: int,
        accumulated_rows: int,
        total_count: int,
        extra_label: str,
    ) -> tuple[float, int]:
        """10분 경과 또는 50,000건 추가 적재마다 하트비트를 로깅하고, 갱신된 (시각, 건수)를 반환한다."""
        current_time = time.time()
        if current_time - last_heartbeat_time < 600 and accumulated_rows - last_heartbeat_rows < 50000:
            return last_heartbeat_time, last_heartbeat_rows

        percent = (accumulated_rows / total_count) * 100 if total_count > 0 else 0
        heartbeat_msg = (
            f"⏳ 전수 수집 진행 중... (현재: {accumulated_rows:,}건 / 총 {total_count:,}건 | {percent:.1f}%)"
        )
        write_progress_log(f"{self.spec.name}{extra_label}", "PROGRESS", heartbeat_msg)
        return current_time, accumulated_rows
    ...
    def _run_single_extraction(
        ...
        buffer_list: list[dict[str, Any]] = []  # DB Flush 용 버퍼
        ...
        while True:
            ...
            if xml_result is None:
                ...
                self._wait_after_error(
                    f"{self.spec.name}{extra_label}",
                    f"⚠️ API 연동 실패 - 30분 대기 후 재시도 예정 (실패 페이지: {page_no})",
                )
                continue
            ...
            if not items:
                action = self._handle_empty_items(
                    page_no, num_of_rows, accumulated_rows, total_count, once, extra_label
                )
                if action == "break":
                    break
                continue
            ...
            self._flush_if_chunk_full(conn, buffer_list)

            self._print_progress_bar(accumulated_rows, total_count, start_time, prefix_msg)

            last_heartbeat_time, last_heartbeat_rows = self._maybe_log_heartbeat(
                last_heartbeat_time, last_heartbeat_rows, accumulated_rows, total_count, extra_label
            )

            if not once:
                self._save_checkpoint(page_no + 1, extra_label)

            if self._should_stop_paging(once, page_no, accumulated_rows, total_count):
                break
        ...
        if not once:
            self._delete_checkpoint(extra_label)
    ...
    def run(self, once: bool = False) -> tuple[int, int]:
        ...
        if self.spec.is_prescription:
            ...
            for month in target_months:
                month_downloaded, month_total = self._run_single_extraction(
                    conn, once, {"yearMonth": month}, f"  ㄴ [{month}] ", extra_label=month
                )
