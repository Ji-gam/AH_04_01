...
import argparse
from typing import Any
from config_db import API_SPECS, DEFAULT_WORKERS, DB_PATH
...
def write_final_summary_report(results: list, once: bool, workers: int):
    ...
    for name, success, downloaded, total, err_msg in sorted(results, key=lambda x: x[0]):
...
def main():
    ...
    parser.add_argument(
        "api_name", 
        nargs="?", 
        help="단일 API 실행 시 API 이름을 입력 (예: e_drug). 입력하지 않으면 전체 실행"
    )
    parser.add_argument(
        "--all", 
        action="store_true", 
        help="config_db.py에 정의된 모든 API를 대상으로 파이프라인 실행"
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="테스트용: 전체를 순회하지 않고 단 1회(1페이지만) 호출 후 종료"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=DEFAULT_WORKERS,
        help=f"병렬 처리할 워커 프로세스 수 (기본값: {DEFAULT_WORKERS})"
    )
    parser.add_argument(
        "--list", 
        action="store_true", 
        help="지원하는 모든 API의 키 목록을 출력합니다."
    )
