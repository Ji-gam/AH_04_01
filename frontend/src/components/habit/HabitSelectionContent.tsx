...
/** 마이다이어리 화면의 본문(아바타+인사/추천 습관 모달 버튼/식단 기록 모달 버튼). 원래
 * HabitSelectionPage 전용이었는데, 홈 화면 라이프스타일 카드를 눌렀을 때 페이지 이동 대신
 * 모달로도 띄울 수 있도록(2026-07-19) 본문만 뽑아냈다 - 페이지 이동(뒤로가기)은 이 컴포넌트를
 * 감싸는 쪽(HabitSelectionPage 또는 HomePage의 Modal)이 책임진다. 추천 습관 리스트와 식단
 * 기록(DietLogContent, F-DIET-1/2)은 둘 다 버튼→모달 방식이다. "선택한 습관"(오늘 진행
 * 체크) 버튼은 여기 있으면 안 예뻐서 뺐다(2026-07-24) - 이미 선택한 습관이 있으면 홈 화면
 * 라이프스타일 카드가 SelectedHabitsModal로 바로 연결되니 기능 자체는 그대로 남아있다. */
