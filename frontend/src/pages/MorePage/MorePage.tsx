            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {results.length > 0 ? (
                results.map((item, index) => (
                  ...
                  >
                    <div style={{ fontWeight: "bold", borderBottom: "1px solid black", paddingBottom: "5px", marginBottom: "5px" }}>
                      {item.item_name} ({item.entp_name})
                    </div>
                    <p style={{ margin: "5px 0" }}>
                      <strong>효능:</strong> {item.efficacy}
                    </p>
                    <p style={{ margin: "5px 0", color: item.precautions.includes("특이사항 없음") ? "black" : "red" }}>
