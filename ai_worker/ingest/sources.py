...
등록 절차는 없다. `source/`에 넣으면 들어간다. RAG 재료가 아닌 것(SQL 조회용 표 등)은
`source/_not_rag/`에 둔다 — 폴더가 갈리므로 "이 파일은 RAG가 아니다"를 선언할 필요가 없다.
...
def _is_source_file(p: Path) -> bool:
    # `_`로 시작하는 건 설정이나 파생물(_tuning.yaml, _not_rag/)이지 원천 데이터가 아니다.
